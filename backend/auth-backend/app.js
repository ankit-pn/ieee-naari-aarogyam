...
// register endpoint
app.post("/register/hospital", (request, response) => {
  // hash the password
  bcrypt
    .hash(request.body.password, 10)
    .then((hashedPassword) => {
...
app.post("/login/hospital", (request, response) => {
  // check if email exists
  Hospital.findOne({ hospitalId: request.body.hospitalId })

    // if email exists
    .then((hospital) => {
      // compare the password entered and the hashed password found
      bcrypt
        .compare(request.body.password, hospital.password)
